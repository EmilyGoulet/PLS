...
class Wpsisac_Pro_Admin {
	...
	function wpsisac_pro_register_menu() {
		// Shortocde Mapper
		add_submenu_page( 'edit.php?post_type='.WPSISAC_PRO_POST_TYPE, __('Shortcode Generator', 'wp-slick-slider-and-image-carousel'), __('Shortcode Generator', 'wp-slick-slider-and-image-carousel'), 'edit_posts', 'wpsisac-shrt-mapper', array($this, 'wpsisac_shortcode_mapper_page') );
	...
	function wpsisac_pro_add_tax_row_data( $actions, $tag ) {
		return array_merge( array( 'wpsisac_pro_id' => "<span style='color:#555'>ID: {$tag->term_id}</span>" ), $actions );
	}
	...
	function wpsisac_pro_plugin_row_meta( $links, $file ) {
		...
		if ( $file == WPSISAC_PRO_PLUGIN_BASENAME ) {
			...
			$row_meta = array(
				'docs'    => '<a href="' . esc_url('http://docs.wponlinesupport.com/wp-slick-slider-and-image-carousel-pro/?utm_source=hp&event=doc') . '" title="' . esc_attr( __( 'View Documentation', 'wp-slick-slider-and-image-carousel' ) ) . '" target="_blank">' . __( 'Docs', 'wp-slick-slider-and-image-carousel' ) . '</a>',
				'support' => '<a href="' . esc_url('https://www.wponlinesupport.com/wordpress-support/?utm_source=hp&event=projobs') . '" title="' . esc_attr( __( 'Premium Support - For any Customization', 'wp-slick-slider-and-image-carousel' ) ) . '" target="_blank">' . __( 'Premium Support', 'wp-slick-slider-and-image-carousel' ) . '</a>',
			);
